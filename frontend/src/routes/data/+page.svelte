<script lang="ts">
  ...
  const tokensLabels = Array.from(new Array(maxTokenCount)).map((_, i) => String(i + 1))

  const humanScatter = word_counts.filter(({ backgroundColor }) => backgroundColor === 'blue').map(({ x, y }) => ({ x: String(x), y }))
  const aiScatter = word_counts.filter(({ backgroundColor }) => backgroundColor === 'red').map(({ x, y }) => ({ x: String(x), y }))
</script>
